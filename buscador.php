...
if (!empty($search) && !empty($location)) {
    $query = "SELECT c.coworking_space_id, c.space_name, c.descripcion, c.Precio, l.name_location, l.address, l.city
...
while ($fila = mysqli_fetch_assoc($result)) {
    ...
    <div class='card mb-4'>
        <div class='card-body'>
            ...
            <p class='card-text'><strong>Ubicación:</strong> {$fila['name_location']} , {$fila['address']} , {$fila['city']}</p>
        </div>
    </div>
</div>";
}
